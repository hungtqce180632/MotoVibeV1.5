    ...
    <body>
        ...
        <div class="container">
            <div class="orders-section">
                <h2 class="section-title"><i class="fas fa-shopping-cart me-2"></i>My Orders</h2>
                ...
                <c:if test="${empty orders}">
                    <div class="alert alert-info">
                        ...
                        <a href="motorList" class="btn btn-primary btn-sm ms-3">Browse Motorcycles</a>
                    </div>
                </c:if>

                <%-- Hiển thị bảng đơn hàng khi có dữ liệu --%>
                <c:if test="${not empty orders}">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                <%-- Lặp qua từng đơn hàng để hiển thị --%>
                                <c:forEach var="o" items="${orders}">
                                    <tr class="order-row" 
                                        data-status="${o.orderStatus.toLowerCase()}"
                                        data-warranty="${o.hasWarranty ? 'yes' : 'no'}">
                                        <td><strong class="order-code">${o.orderCode}</strong></td>
                                        <td>${o.motorName}</td>
                                        <td>
                                            <%-- Hiển thị trạng thái đơn hàng với màu sắc và biểu tượng tương ứng --%>
                                            <span class="status-badge
                                                  ${o.orderStatus eq 'Pending' ? 'status-pending' : 
                                                    o.orderStatus eq 'Shipped' ? 'status-shipped' : 
                                                    o.orderStatus eq 'Completed' ? 'status-Completed' : 
                                                    'status-cancelled'}">
                                                <i class="fas
                                                   ${o.orderStatus eq 'Pending' ? 'fa-clock' : 
                                                     o.orderStatus eq 'Shipped' ? 'fa-truck' : 
                                                     o.orderStatus eq 'Completed' ? 'fa-check-circle' : 
                                                     'fa-times-circle'} me-1"></i>
                                                   ${o.orderStatus}
                                                </span>
                                            </td>
                                            <%-- Hiển thị tổng số tiền đã định dạng --%>
                                            <td><fmt:formatNumber value="${o.totalAmount}" type="currency" currencySymbol="$" maxFractionDigits="2"/></td>
                                            <td>
                                            ...
                                            <td>
                                                <%-- Nút để xem chi tiết bảo hành nếu có --%>
                        ...
                        button.addEventListener('click', function () {
                            ...
                            orderRows.forEach(row => {
                                if (filter === 'all') {
                                    ...
                                    if (row.getAttribute('data-warranty') === 'yes') {
                                        row.style.display = '';
                                    } else {
                                    ...
                                    if (statusText.toLowerCase() === filter.toLowerCase()) {
                                        row.style.display = '';
                                    } else {
                                        row.style.display = 'none';
                                    }
