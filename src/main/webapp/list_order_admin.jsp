    ...
    <head>
        ...
        <style>
            :root {
                ...
                --rich-black: #1A1A1A;
                --text-gold: #F5E6CC;
                --light-gold: #FFF9B0; /* Màu vàng nhạt mới */
            }
                                ...
                                <tbody>
                                    <c:forEach var="order" items="${orders}">
                                        <tr>
                                            ...
                                            <td>${not empty order.customerName ? order.customerName : 'N/A'}
                                            </td>
                                            <td>${not empty order.employeeName ? order.employeeName : 'N/A'}
                                            </td>
                                            <td>${not empty order.motorName ? order.motorName : 'N/A'}</td>
                                            <td>
                                                <c:choose>
                                                    <c:when test="${not empty order.createDate}">
                                            ...
                                            <td>${not empty order.paymentMethod ? order.paymentMethod : 'N/A'}
                                            </td>
                                            <td>${order.totalAmount != null ? order.totalAmount : '0.00'}</td>
                                            <td>
                                                <c:choose>
                                                    <c:when
                                                        test="${order.depositStatus != null && order.depositStatus}">
                                                        Yes
                                                    </c:when>
                                                    <c:otherwise>
                                                        No
                                                ...
                                                <td>
                                                    <c:if test="${sessionScope.user.role eq 'admin'}">
                                                        ...
                                                        <c:if test="${order.depositStatus eq false}">
                                                            <div id="confirm-deposit-${order.orderId}"
                                                                 class="confirm-deposit-container">
                                                                <form action="confirmDeposit" method="post">
                                                                    <input type="hidden" name="orderId"
                                                                           value="${order.orderId}">
                                                                    <button type="submit"
                                                                            class="btn-confirm-deposit">Confirm
                                                                        Deposit</button>
                                                                </form>
                                                            </div>
                                                        </c:if>
                                                            ...
                                                            <div id="warranty-${order.orderId}"
                                                                ...
                                                                <form action="viewOrderWarranty" method="get">
                                                                    <input type="hidden" name="orderId"
                                                                           value="${order.orderId}">
                                                                    <button type="submit"
                                                                            class="btn-detail-warranty">Detail
                                                                        Warranty</button>
                                                                </form>
                                                            </div>
                                                        </c:if>
                                                    ...
                                                    <c:if test="${sessionScope.user.role eq 'employee'}">
                                                        <c:if
                                                            test="${order.depositStatus eq true && order.orderStatus eq 'Shipped'}">
                                                            <div id="confirm-deposit-${order.orderId}"
                                                                 class="confirm-deposit-container">
                                                                <form action="ConfirmOrderEmployeeServlet"
                                                                      method="post">
                                                                    <input type="hidden" name="orderId"
                                                                           value="${order.orderId}">
                                                                    <button type="submit"
